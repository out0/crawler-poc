...
template <typename T>
cv::Mat to_matrix(T *data, int rows, int cols, int channels)
{
	// Create Mat from buffer
	cv::Mat mat = cv::Mat(rows, cols, CV_MAKETYPE(cv::DataType<T>::type, channels));
	memcpy(mat.data, data, rows * cols * channels * sizeof(T));
	return mat;
}

template <typename T>
T *to_array(cv::Mat mat)
{
	int ch = mat.channels();
	T *result = new T[mat.rows * mat.cols * ch];
...
int main(int argc, char const *argv[])
{
	...
	while (true)
	{
		...
		cv::cuda::GpuMat gpuInput = cv::cuda::GpuMat(source);

		//cv::cuda::warpPerspective(gpuInput, gpuOutput, transformationMat, cv::Size(source.rows, source.cols), INTER_LINEAR, BORDER_CONSTANT, 0, cv::cuda::Stream::Null());
		cv::cuda::warpPerspective(gpuInput, gpuOutput, transformationMat, image_size, INTER_CUBIC | WARP_INVERSE_MAP, BORDER_CONSTANT, 0, cv::cuda::Stream::Null());

		gpuOutput.download(destination);
		gpuOutput.release();
		gpuInput.release();
